#' Plot emotional arc
  ...
  names(sentiment_tibble)) {
      rlang::abort("Input tibble must contain 'chapter' and 'sentiment_score'
  columns.")
    }

    if (!is.integer(sentiment_tibble$chapter)) {
      rlang::abort("'chapter' column must be of type integer.")
    }

    if (!is.numeric(sentiment_tibble$sentiment_score)) {
      rlang::abort("'sentiment_score' column must be of type numeric.")
    }
